...
import org.json4s.{JArray, JBool, JDouble, JInt}
...
class ValueConversionTest extends AnyFreeSpec with Matchers {
  "Value conversion" - {
    ...
    "Converts List correctly" in {
      val listExample1 = ValueConversion.anyToJValue(List(true, false, 22, "Hello"))
      listExample1 should matchPattern {
        case JArray(List(JBool(true), JBool(false), _, JString("Hello"))) =>
      }
      val listExample2 = ValueConversion.anyToJValue(List("Hello", "World", "123"))
