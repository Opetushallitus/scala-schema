...
object AnyOfExtractor {
  ...
  trait DiscriminatorCriterion {
    def keyPath: KeyPath
    def apply(value: JsonCursor)(implicit context: ExtractionContext): List[String]
    def matches(json: JsonCursor)(implicit context: ExtractionContext) = apply(json).isEmpty
  ...
  case class AllOfCriterion(criteria: List[DiscriminatorCriterion]) extends DiscriminatorCriterion{
    ...
    def apply(json: JsonCursor)(implicit context: ExtractionContext): List[String] = criteria.flatMap(c => c.apply(json))
  ...
  case class OneOfCriteria(criteria: List[DiscriminatorCriterion]) extends DiscriminatorCriterion {
    ...
    def apply(json: JsonCursor)(implicit context: ExtractionContext): List[String] = if (criteria.find(_.apply(json).isEmpty).isDefined) Nil else List(description)
  ...
  case class Flattened(val keyPath: KeyPath, schema: FlattenedSchema, wrappedSchemaCriterion: DiscriminatorCriterion) extends DiscriminatorCriterion {
    ...
    def apply(json: JsonCursor)(implicit context: ExtractionContext): List[String] = wrappedSchemaCriterion.apply(json)
  }

  case class PropertyExists(val keyPath: KeyPath) extends DiscriminatorCriterion {
    def apply(value: JsonCursor)(implicit context: ExtractionContext): List[String] = keyPath(value).json match {
  ...
  abstract class TypeMatcher(val description: String) extends DiscriminatorCriterion {
    override def apply(value: JsonCursor)(implicit context: ExtractionContext): List[String] = matchType(value) match {
      case true => Nil
      case _ => List(description)
    }
    def matchType(value: JsonCursor)(implicit context: ExtractionContext): Boolean
    def weight = 100
  }
  ...
  case class IsDate(val keyPath: KeyPath, schema: DateSchema) extends TypeMatcher("date") {
    def matchType(value: JsonCursor)(implicit context: ExtractionContext): Boolean = DateExtractor.extractDate(value, schema, Nil).isRight
  }

  case class IsArray(val keyPath: KeyPath, schema: ListSchema) extends TypeMatcher("array") {
    def matchType(value: JsonCursor)(implicit context: ExtractionContext): Boolean = value.json match {
  ...
  case class NotWhenCriteria(keyPath: KeyPath, criteria: List[SerializableNotWhen]) extends DiscriminatorCriterion {
    ...
    def apply(json: JsonCursor)(implicit context: ExtractionContext): List[String] = {
      val valueAtKeyPath = keyPath(json)
      val found = criteria.flatMap { case SerializableNotWhen(path, values) =>
        val valueAtExpectedPosition = valueAtKeyPath.navigate(path).json
        val matches = values match {
          case Some(a) => a.filter(anyValue => JsonCompare.equals(valueAtExpectedPosition, ValueConversion.anyToJValue(anyValue)))
          case None => Nil
        }
        matches
      }
      found match {
        case List(_) => Nil
        case Nil => List(description)
      }
    }

    override def description: String = {
      def convertToString(arr: Option[JArray]) = arr match {
        case Some(jarr) => jarr.values.map(someValue => JsonMethods.compact(ValueConversion.anyToJValue(someValue))).mkString(",")
        case None => ""
      }
      criteria.map { case SerializableNotWhen(path, values) => s"${path}=${convertToString(values)}" }.mkString(" or ")
    }
  }
