...
case class TooManyMatchingCasesException(path: String, cases: List[(SchemaWithClassName, DiscriminatorCriterion)], json: JValue) extends RuntimeException(s"Deserialization error at ${path}: more than 1 matching case: \n${cases.map {
  case (schema, criteria) => s"${schema.simpleName} (${criteria})"
