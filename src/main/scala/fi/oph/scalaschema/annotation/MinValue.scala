...
object MinValue extends MetadataSupport[MinValue] {
  ...
  override def appendMetadataToJsonSchema(obj: JObject, metadata: MinValue) = appendToDescription(obj.merge(JObject("minimum" -> JDouble(metadata.value))), "(Minimum value: " + metadata.value + ")")
}
